...
import { ManifestSchema } from '../../schemas/v1/manifest';
 ...
 *
 * @remarks
 * This function accepts a message string and constructs a new Error object prefixed with "[DotLottieV1-js]: ".
 *
 * @deprecated
 * This function has been deprecated in favor of using the {@link DotLottieV1Error} class directly.
 *
 * @param message - The error message to include in the Error object.
 * @returns An Error object with the specified message, prefixed with "[DotLottieV1-js]: ".
 *
 * @example
 * ```typescript
 * const message = 'DotLottieV1 not found';
...
export const createError = (message: string): Error => {
  const error = new Error(`[DotLottieV1-js]: ${message}`);
...
export const getMimeTypeFromBase64 = (base64: string): string | undefined => {
  ...
  if (!base64) {
    throw new DotLottieV1Error(
      'Failed to determine the MIME type from the base64 asset string. Please check the input data. Supported asset types for DotLottieV1-js  are: jpeg, png, gif, bmp, svg, webp, mp3',
 ...
 *
 * @param DotLottieV1 - The .lottie data as a Uint8Array.
 ...
 * @throws {@link DotLottieV1Error} if the .lottie data is not provided or is invalid.
 *
 * @example
 * ```typescript
 * const DotLottieV1 = new Uint8Array(...);
 * const unzippedData = await unzipDotLottieV1(DotLottieV1);
 * ```
 *
 * @public
 */
export async function unzipDotLottieV1(
  DotLottieV1: Uint8Array | undefined,
  filter: UnzipFileFilter = (): boolean => true,
): Promise<Unzipped> {
  if (!(DotLottieV1 instanceof Uint8Array)) {
    throw new DotLottieV1Error('DotLottieV1 not found', ErrorCodes.INVALID_DOTLOTTIE);
  }

  const unzipped = await new Promise<Unzipped>((resolve, reject) => {
    fflateUnzip(DotLottieV1, { filter }, (err, data) => {
      if (err) {
        reject(err);
      }
      resolve(data);
 ...
 *
 * @param DotLottieV1 - The `.lottie` file content as a `Uint8Array`.
 ...
 *
 * @throws {@link DotLottieV1Error} if the input is not a valid `.lottie` file or if the target file is not found.
 *
 * @example
 * ```typescript
 * const DotLottieV1 = new Uint8Array(...);
 * const targetPath = 'images/image.png';
 * const unzippedFile = await unzipDotLottieV1File(DotLottieV1, targetPath);
 * ```
 *
 * @public
 */
export async function unzipDotLottieV1File(
  DotLottieV1: Uint8Array,
  path: string,
  ...
  const unzipped = await unzipDotLottieV1(DotLottieV1, (file) => file.name === path && (!filter || filter(file)));

  return unzipped[path];
}

/**
 * Retrieves the manifest data from the given DotLottieV1 object.
 *
 * @remarks
 * This function accepts a DotLottieV1 object as a Uint8Array and extracts the manifest data from it.
 ...
 *
 * @param DotLottieV1 - The Uint8Array of DotLottieV1 data.
 * @returns A Promise that resolves with the manifest data or `undefined` if not found.
 *
 * @example
 * ```typescript
 * const DotLottieV1 = new Uint8Array(...);
 * const manifestData = await getManifest(DotLottieV1);
 * ```
 *
 * @public
 */
export async function getManifest(DotLottieV1: Uint8Array): Promise<Manifest | undefined> {
  const manifestFileName = 'manifest.json';

  const unzipped = await unzipDotLottieV1(DotLottieV1, (file) => file.name === manifestFileName);
...
/**
 * Validates the provided DotLottieV1 data.
 *
 * @remarks
 ...
 *
 * @param DotLottieV1 - The DotLottieV1 data as a Uint8Array.
 * @returns A Promise that resolves with an object containing a success boolean and an optional error string.
 *
 * @example
 * ```typescript
 * const DotLottieV1 = new Uint8Array(...);
 * const validationResult = await validateDotLottieV1(DotLottieV1);
 * ```
 *
 * @public
 */
export async function validateDotLottieV1(dotLottieV1: Uint8Array): Promise<{ error?: string; success: boolean }> {
  if (!(dotLottieV1 instanceof Uint8Array)) {
    return { success: false, error: 'DotLottieV1 not found' };
  }

  const manifest = await getManifest(dotLottieV1);
  ...
  if (!manifestValidationResult.success) {
    const error = `Invalid .lottie file, manifest.json structure is invalid, ${JSON.stringify(
      flatten(manifestValidationResult.issues).nested,
      null,
      2,
    )}`;
 ...
 * It validates the data and returns a Promise that resolves with the DotLottieV1 data as a Uint8Array.
 *
 * @param arrayBuffer - The ArrayBuffer containing .lottie data.
 * @returns A Promise that resolves with the DotLottieV1 data as a Uint8Array.
 * @throws {@link DotLottieV1Error} if the data is invalid.
 *
 * @example
 * ```typescript
 * const arrayBuffer = new ArrayBuffer(...);
 * const DotLottieV1 = await loadFromArrayBuffer(arrayBuffer);
 * ```
 *
 * @public
 */
export async function loadFromArrayBuffer(arrayBuffer: ArrayBuffer): Promise<Uint8Array> {
  const DotLottieV1 = new Uint8Array(arrayBuffer);

  const validationResult = await validateDotLottieV1(DotLottieV1);

  if (validationResult.error) {
    throw new DotLottieV1Error(validationResult.error, ErrorCodes.INVALID_DOTLOTTIE);
  }
 ...
 * It returns a Promise that resolves with the DotLottieV1 data as a Uint8Array.
 *
 * @param src - The URL source of the .lottie file.
 * @returns A Promise that resolves with the DotLottieV1 data as a Uint8Array.
 * @throws {@link DotLottieV1Error} if the URL is invalid or if the content type is incorrect.
 *
 * @example
 * ```typescript
 * const url = 'https://example.com/animation.lottie';
 * const DotLottieV1 = await loadFromURL(url);
 * ```
 *
 * @public
 */
export async function loadFromURL(src: string): Promise<Uint8Array> {
  if (!isValidURL(src)) {
    throw new DotLottieV1Error('Invalid url provided for .lottie file', ErrorCodes.INVALID_URL);
  }
...
/**
 * Retrieves an audio from the given DotLottieV1 object by its filename.
 *
 * @remarks
 * This function accepts a DotLottieV1 object as a Uint8Array, the filename of the audio to retrieve, and an optional filter function.
 ...
 * const imageData = await getAudio(DotLottieV1, filename);
 * ```
 *
 * @public
 */
export async function getAudio(
  ...
  const unzipped = await unzipDotLottieV1File(DotLottieV1, audioFilename, filter);
...
/**
 * Retrieves all audio files from the given DotLottieV1 object.
 *
 * @remarks
 * This function accepts a DotLottieV1 object as a Uint8Array and an optional filter function to further refine the extraction.
 ...
 * ```typescript
 * const DotLottieV1 = new Uint8Array(...);
 * const allAudio = await getAllAudio(DotLottieV1);
 * ```
 *
 * @public
 */
export async function getAllAudio(DotLottieV1: Uint8Array, filter?: UnzipFileFilter): Promise<Record<string, string>> {
  const unzippedAudio = await unzipDotLottieV1(DotLottieV1, (file) => {
...
/**
 * Inlines audio assets for the given animations within a DotLottieV1 object.
 *
 * @remarks
 * This function accepts a DotLottieV1 object as a Uint8Array and a record containing the animations to process.
 ...
 *
 * @param DotLottieV1 - The DotLottieV1 object containing the animations.
 ...
 * await inlineAudioAssets(DotLottieV1, animations);
 * ```
 *
 * @public
 */
export async function inlineAudioAssets(
  ...
  const unzippedAudio = await getAllAudio(DotLottieV1, (file) => audioMap.has(file.name));
...
/**
 * Retrieves an image from the given DotLottieV1 object by its filename.
 *
 * @remarks
 * This function accepts a DotLottieV1 object as a Uint8Array, the filename of the image to retrieve, and an optional filter function.
 ...
 * const imageData = await getImage(DotLottieV1, filename);
 * ```
 *
 * @public
 */
export async function getImage(
  ...
  const unzipped = await unzipDotLottieV1File(DotLottieV1, imageFilename, filter);
...
/**
 * Retrieves all images from the given DotLottieV1 object.
 *
 * @remarks
 * This function accepts a DotLottieV1 object as a Uint8Array and an optional filter function to further refine the extraction.
 ...
 * ```typescript
 * const DotLottieV1 = new Uint8Array(...);
 * const images = await getImages(DotLottieV1);
 * ```
 *
 * @public
 */
export async function getImages(DotLottieV1: Uint8Array, filter?: UnzipFileFilter): Promise<Record<string, string>> {
  const unzippedImages = await unzipDotLottieV1(DotLottieV1, (file) => {
...
/**
 * Inlines image assets for the given animations within a DotLottieV1 object.
 *
 * @remarks
 * This function accepts a DotLottieV1 object as a Uint8Array and a record containing the animations to process.
 ...
 * const DotLottieV1 = new Uint8Array(...);
 * const animations = { animation1: {...}, animation2: {...} };
 * await inlineImageAssets(DotLottieV1, animations);
 * ```
 *
 * @public
 */
export async function inlineImageAssets(
  ...
  const unzippedImages = await getImages(DotLottieV1, (file) => imagesMap.has(file.name));
...
/**
 * Retrieves an animation from the given DotLottieV1 object by its ID.
 *
 * @remarks
 * This function accepts a DotLottieV1 object as a Uint8Array, the animation ID to retrieve, and an optional inlineAssets option.
 ...
 * const animationData = await getAnimation(DotLottieV1, animationId, { inlineAssets: true });
 * ```
 *
 * @public
 */
export async function getAnimation(
  ...
  const unzippedAnimation = await unzipDotLottieV1File(DotLottieV1, animationFilename, filter);
  ...
  await inlineImageAssets(DotLottieV1, animationsMap);

  await inlineAudioAssets(DotLottieV1, animationsMap);

  return animationData;
}

/**
 * Retrieves the animations from the given DotLottieV1 object, with optional filtering and asset inlining.
 *
 * @remarks
 * This function accepts a DotLottieV1 object as a Uint8Array, an optional inlineAssets option, and an optional filter function.
 ...
 * ```typescript
 * const DotLottieV1 = new Uint8Array(...);
 * const animations = await getAnimations(DotLottieV1, { inlineAssets: true });
 * ```
 *
 * @public
 */
export async function getAnimations(
  ...
  const unzippedAnimations = await unzipDotLottieV1(DotLottieV1, (file) => {
